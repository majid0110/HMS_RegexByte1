...
class ReportsController extends Controller
{
    ...
    public function generateExcelServiceReport()
    {
        ...
        $toDate = $this->request->getPost('toDate');
        $invoice = $this->request->getPost('invoice');
        $sales = $salesModel->getSalesReport($search, $invoice, $paymentInput, $clientName, $fromDate, $toDate);
        ...
        $sheet->setCellValue('A2', "Generated by: $generatedBy");
        $sheet->getStyle('A2')->applyFromArray($headerStyle);
        $sheet->setCellValue('D2', 'Generated On: ' . date('Y-m-d H:i:s'));
        $sheet->getStyle('A2')->applyFromArray($headerStyle);
        $sheet->mergeCells('A2:C2');
        ...
        if (!empty($invoice)) {
            $sheet->setCellValue('A' . $filterRow, 'Filter by Invoice: ' . $invoice);
            $filterRow++;
        }

        $headers = ['Invoice Number', 'Client Name', 'Currency', 'Payment Method', 'Status', 'Date', 'Value'];
        ...
        foreach ($sales as $Sale) {
            $sheet->setCellValue('A' . $row, $Sale['invOrdNum']);
            $sheet->setCellValue('B' . $row, $Sale['clientName']);
            $sheet->setCellValue('C' . $row, $Sale['Currency']);
            $sheet->setCellValue('D' . $row, $Sale['PaymentMethod']);
            $sheet->setCellValue('E' . $row, $Sale['Status']);
            $sheet->setCellValue('F' . $row, $Sale['Date']);
            $sheet->setCellValue('G' . $row, $Sale['Value']);
        ...
        // Merge cells for total value
        $sheet->mergeCells('F' . $lastRow . ':F' . $lastRow);

        $writer = new Xlsx($spreadsheet);
        $filename = 'services_report.xlsx';
