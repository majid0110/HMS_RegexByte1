...
<body style="background: #bf4d0a8c;  background-repeat: no-repeat; background-size: cover;">
                        ...
                        <div class="item-list-container">
                            <div class="item-list">
                                <?php foreach ($items as $item):
                                    ...
                                    $serviceExpiries = $PurchaseModel->getServiceExpiry($item['idItem'], $businessID);
                                    ?>
                                    <div class="item" style="align-content: center;"
                                        data-service-id="<?= $item['idItem']; ?>" data-service-price="<?= $item['Cost']; ?>"
                                        data-service-tax-id="<?= $item['idTVSH']; ?>"
                                        data-service-tax-value="<?= $item['tax_value']; ?>"
                                        data-has-expiry="<?= count($serviceExpiries) > 0 ? '1' : '0' ?>">
                                        <div style="font-weight: bolder;"><?= $item['Name']; ?></div>
                                        <div style="margin-bottom:auto; font-size: small;"><?= $item['Cost']; ?> pkr
                                        </div>
...
<tr data-service-id="${serviceId}">
    ...
    <div class="quantity-input" style="display: flex;flex-direction: row;justify-content: space-evenly;">
        ...
        <input type="text" class="editable-quantity form-control quantity-box" style="width: 60px; padding: 2%;" value="1.00">
    ...
    <td><button class="delete-button">Delete</button></td>
            ...
            function filterServices(categoryId) {
                ...
                $.ajax({
                    url: '<?php echo base_url() . "PurchaseController/filteritems"; ?>',
                    type: 'POST',
            ...
            function insertData() {
                ...
                $.ajax({
                    method: 'POST',
                    url: '<?= site_url('submitPurchaseServices') ?>',
                    dataType: "json",
                    data: {
                        clientId: clientId,
            ...
            function submitInvoice() {
                ...
                $.ajax({
                    method: 'POST',
                    url: '<?= site_url('submitPurchaseInvoice') ?>',
                    dataType: "json",
                    data: {
                        clientId: clientId,
                        clientName: clientName,
